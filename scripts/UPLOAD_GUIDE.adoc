...
Upload in this order to set up the ecosystem properly: 1. GrimGreaser
(build system) 2. GrimPager (browsing helper) 3. GrimTemplateEngine
(main tool) 4. GrimLicenseChecker (validation) 5. GrimCIValidator
(validation) 6. GrimSecurityScanner (validation)
...
For each script: - [ ] Go to https://greasyfork.org/en/scripts/new - [ ]
Click "`Choose File`" and select the .user.js file - [ ] Set Language to
"`English`" - [ ] Paste the description - [ ] Add the tags
(space-separated) - [ ] Click "`Post script`" - [ ] Go to script edit
page - [ ] Scroll to "`Sync`" section - [ ] Configure webhook settings -
[ ] Click "`Update script`"
